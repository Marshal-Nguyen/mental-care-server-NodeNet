...
exports.getAllDoctorProfiles = async (
    ...
    if (specialtiesId) {
        query = query
            .filter("DoctorProfileSpecialty.SpecialtiesId", "eq", specialtiesId)
            .not("DoctorProfileSpecialty", "is", null);
    }
...
exports.searchDoctorProfilesByName = async (
    fullName = "",
    pageIndex = 1,
    pageSize = 10,
    sortBy = "FullName",
...
exports.updateDoctorProfile = async (id, profileData) => {
    ...
    if (specialties && Array.isArray(specialties)) {
        ...
        const validSpecialtiesIds = specialties
            .map((spec) => spec.Id || spec.id)
            .filter((id) => id);
        if (validSpecialtiesIds.length > 0) {
            ...
            if (specialtyInserts.length > 0) {
                await supabase.from("DoctorProfileSpecialty").upsert(specialtyInserts, {
                    onConflict: ["DoctorProfilesId", "SpecialtiesId"],
                });
            }
        }
    }
